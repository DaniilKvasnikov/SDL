					...
					t_element *(*active_other)(t_element *elem))
{
	...
	if (fined_element_by_name(win, name))
		ui_fotal_error(name, "element duplicate");
	elem = (t_element *)ui_checkmalloc(malloc(sizeof(t_element)), "elem init");
	elem->active = active;
	elem->name = ft_strdup(name);
	elem->rect = rect;
	elem->win = win;
	elem->last_delta_mouse = (t_point){0, 0};
	elem->draw_sub_groupe = (t_element **)ui_checkmalloc(malloc(sizeof(t_element *)), "draw_sub_groupe");
	elem->draw_sub_groupe[0] = NULL;
	elem->sub_groupe = (t_element **)ui_checkmalloc(malloc(sizeof(t_element *)), "sub_groupe");
	elem->sub_groupe[0] = NULL;
	elem->parent = parent;
	elem->color = color;
	elem->str = str;
	elem->ttf = ttf;
	elem->params = (t_param **)ui_checkmalloc(malloc(sizeof(t_param *)), "params");
	elem->params[0] = NULL;
