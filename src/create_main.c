...
/*   Updated: 2019/08/21 17:47:42 by rrhaenys         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	*create_main(t_mydata *mydata, char *name)
{
	...
	t_element *img = init_image_to_win((t_rect){menulist->pos.x + menulist->size.x / 10, menulist->pos.y + menulist->size.y / 4, menulist->size.x / 5, menulist->size.y / 2}, "./img/cat.bmp", main, NULL, NULL, NULL, NULL, &button_render);
	menulist->sub_group = add_group_e(menulist->sub_group, img, NULL);
	img->parent = menulist;
	t_element *but1 = init_button_to_win((t_rect){menulist->pos.x, menulist->pos.y + menulist->size.y, menulist->size.x, menulist->size.y}, "./img/button_1.bmp", main, "Button1", &element_touch, NULL, &button_render);
	t_element *but2 = init_button_to_win((t_rect){menulist->pos.x, menulist->pos.y + menulist->size.y * 2, menulist->size.x, menulist->size.y}, "./img/button_1.bmp", main, "Button2", &element_touch, NULL, &button_render);
	...
	t_element *winlist_sub = init_menulist_to_win((t_rect){winlist->pos.x, winlist->pos.y + winlist->size.y, winlist->size.x, winlist->size.y}, "./img/button_1.bmp", main, "Sub", &menulist_touch, &active_elem, &dropdownlist_render);
	winlist->sub_group = add_group_e(winlist->sub_group, winlist_sub, NULL);
	winlist_sub->parent = winlist;

	t_element *but3 = init_button_to_win((t_rect){winlist_sub->pos.x + winlist_sub->size.x, winlist_sub->pos.y, winlist_sub->size.x, winlist_sub->size.y}, "./img/button_1.bmp", main, "Button3", &element_touch, NULL, &button_render);
	winlist_sub->sub_group = add_group_e(winlist_sub->sub_group, but3, NULL);
	but3->parent = winlist_sub;
	t_element *img2 = init_image_to_win((t_rect){but3->pos.x + but3->size.x / 10, but3->pos.y + but3->size.y / 4, but3->size.x / 5, but3->size.y / 2}, "./img/cat.bmp", main, NULL, NULL, NULL, NULL, &button_render);
