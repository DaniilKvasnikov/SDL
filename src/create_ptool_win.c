	...
	*create_ptool_win(t_mydata *mydata, char *name)
{
	...
	img = init_image_to_win((t_rect){elem->rect.x + elem->rect.w / 2 - elem->rect.w / 4, elem->rect.y + elem->rect.h / 2 - elem->rect.h / 4, elem->rect.w / 2, elem->rect.h / 2}, "img/figures.bmp", win, NULL, NULL, NULL, NULL, &button_render);
	elem->sub_group = add_group_e(elem->sub_group, img, NULL);
	img->parent = elem;
	return (win);
}
